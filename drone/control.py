...
class Control:
    ...
    • Telemetry Helpers
    """
    def __init__(self, connection: Connection, telemetry: Telemetry = None, message_receiver: MessageReceiver = None):
        """
        Create a new flight controller.
        Parameters: 
            connection: Active MAVLink connection.
            telemetry: Optional Telemetry instance for emergency operations.
            message_receiver: Optional central message receiver for efficient message handling.
        """
    ...
    def _wait_for_mode(self, mode, timeout=20):
        ...
        while time.time() - start < timeout:
            # Try to get heartbeat from message receiver cache first
            if self.message_receiver:
                hb = self.message_receiver.get_cached_message("HEARTBEAT")
                if hb is None:
                    # Fallback to direct recv_match if cache is empty
    ...
    def set_mode(self, mode):
       ...
       self.master.mav.command_long_send(
           ...
           0, 1, mode_id,  # param2: custom mode (flight mode number)
           0,  # param3: not used
           0,  # param4: not used
           0,  # param5: not used
           0,  # param6: not used
    ...
    def takeoff(self, altitude):
        ...
        elif altitude >= config.flight.MAX_ALTITUDE:
            raise ControlError("Takeoff altitude exceeds maximum allowed altitude.", 
                            details={"altitude": altitude, "max_altitude": config.flight.MAX_ALTITUDE})
...
if __name__ == "__main__":
    ...
    try:
        connection.connect()
        control = Control(connection, telemetry)
        control.set_mode("GUIDED")
        control.arm()
        control.takeoff(5)
        telemetry.wait_until_altitude(5)
        logger.info(f"Battery: {telemetry.get_battery()}%")
        logger.info(f"Heading: {telemetry.get_heading()}°")
        time.sleep(5)
        control.land()
