...
class Failsafe:
    ...
    def __init__(self, connection: Connection, control: Control, telemetry: Telemetry, message_receiver: MessageReceiver = None):
        """
        ...
        Parameters:
            ...
            message_receiver: Optional central message receiver for event-driven monitoring
        """
        ...
        # Subscribe to critical messages if receiver is provided
    ...
    def _setup_message_subscriptions(self):
        """
        Subscribe to critical MAVLink messages for event-driven monitoring.
        """
        self.message_receiver.subscribe('HEARTBEAT', self._handle_heartbeat_message)
        self.message_receiver.subscribe('SYS_STATUS', self._handle_sys_status_message)
        logger.debug("Failsafe message subscriptions set up")
    
    def _cleanup_message_subscriptions(self):
        """
        Unsubscribe from critical MAVLink messages.
        """
        if self.message_receiver:
            self.message_receiver.unsubscribe('HEARTBEAT', self._handle_heartbeat_message)
            self.message_receiver.unsubscribe('SYS_STATUS', self._handle_sys_status_message)
            logger.debug("Failsafe message subscriptions cleaned up")
    
    def _handle_heartbeat_message(self, msg):
        """Handle HEARTBEAT message for link monitoring."""
        self._last_heartbeat_time = time.monotonic()
        # Check link status on each heartbeat
        link_status = self._check_link()
        if link_status:
            self._handle_failsafe(*link_status)
    
    def _handle_sys_status_message(self, msg):
        """Handle SYS_STATUS message for battery monitoring."""
        battery_status = self._check_battery_from_message(msg)
        if battery_status:
            self._handle_failsafe(*battery_status)
    
    def update_heartbeat(self):
        """Update the last heartbeat timestamp to current time (legacy support)."""
        self._last_heartbeat_time = time.monotonic()
    ...
    def start_monitoring(self):
        ...
        # If using message receiver, monitoring is event-driven
        if self.message_receiver:
            self._monitoring_active = True
            logger.info("Event-driven failsafe monitoring started via message receiver")
            return
        
        # Fallback to polling mode for backward compatibility
        self._monitoring_active = True
        ...
        logger.info("Background failsafe monitoring started (polling mode)")
    ...
    def _check_battery_from_message(self, msg) -> Optional[tuple]:
        """
        Check battery level from SYS_STATUS message.
        Returns:
            Optional[tuple] - (FailsafeLevel, reason) if threshold exceeded, None otherwise
        """
        try:
            ...
            if battery < 0:
                return None  # Battery information unavailable
            if battery <= config.failsafe.BATTERY_EMERGENCY:
                return (FailsafeLevel.EMERGENCY, f"Critical battery: {battery}%")
            elif battery <= config.failsafe.BATTERY_CRITICAL:
                return (FailsafeLevel.CRITICAL, f"Low battery: {battery}%")
            ...
            logger.error(f"Battery check from message failed: {e}")
            return None
    ...
    def _check_link(self) -> Optional[tuple]:
        ...
        """
        try:
            time_since_heartbeat = time.monotonic() - self._last_heartbeat_time
