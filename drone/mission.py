...
class MissionPlanner:
    ...
    def load_mission(self, filename: str) -> bool:
        ...
        """
        # Input validation - sanitize filename
        ...
        if '..' in filename or '/' in filename or '\\' in filename:
            logger.error(f"Invalid filename (path traversal attempt): {filename}")
            return False
        ...
        # Ensure resolved path is within mission directory
        mission_file = mission_file.resolve()
        mission_dir_resolved = self.mission_dir.resolve()
        if not str(mission_file).startswith(str(mission_dir_resolved)):
            logger.error(f"Invalid file path (outside mission directory): {mission_file}")
            return False
        ...
        try:
            ...
            if 'waypoints' not in mission_data:
                logger.error("Mission file missing 'waypoints' key")
                return False
            
            if not isinstance(mission_data['waypoints'], list):
                logger.error("Mission file 'waypoints' must be a list")
                return False
            ...
            self.navigation.clear_waypoints()
            self.navigation.waypoints.extend(new_waypoints)
            
            if config.mission.VALIDATE_ON_LOAD:
                validation_result = self.validate_mission()
                if not validation_result['valid']:
                    logger.warning(f'Mission validation failed on load: {validation_result["errors"]}')
                    self.navigation.clear_waypoints()
                    return False
            
            self.current_mission_file = mission_file
            logger.info(f"Mission loaded from: {mission_file} with {len(new_waypoints)} waypoints")
            return True
    ...
    def save_mission(self, filename: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            if config.mission.VALIDATE_ON_SAVE:
                validation_result = self.validate_mission()
                if not validation_result['valid']:
                    logger.warning(f"Mission has validation issues: {validation_result['errors']}")
                    return False
            # Save to file
    ...
    def validate_mission(self) -> Dict[str, Any]:
        ...
        for i, waypoint in enumerate(self.navigation.waypoints):
            ...
            if not config.flight.MIN_LATITUDE <= waypoint.latitude <= config.flight.MAX_LATITUDE:
                errors.append(f"Waypoint {i+1}: Invalid latitude {waypoint.latitude}")
            
            # Check longitude range
            if not config.flight.MIN_LONGITUDE <= waypoint.longitude <= config.flight.MAX_LONGITUDE:
                errors.append(f"Waypoint {i+1}: Invalid longitude {waypoint.longitude}")
            
            # Check altitude range (assuming reasonable limits)
            if waypoint.altitude < 0 or waypoint.altitude > config.flight.MAX_ALTITUDE:
