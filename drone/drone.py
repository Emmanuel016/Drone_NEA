...
class Drone:
    ...
    def __init__(self):
        ...
        self.message_receiver: Optional[MessageReceiver] = None
    ...
    def Start(self, connection_string: str = None, 
        ...
        try:
            ...
            # Create central message receiver
            self.message_receiver = MessageReceiver(self.connection)
            self.message_receiver.start()
            logger.info("Central message receiver started")
            
            # Create subsystems with dependencies (using central message receiver)
            self.telemetry = Telemetry(self.connection, self.message_receiver)
            self.control = Control(self.connection, self.telemetry, self.message_receiver)
            self.camera = Camera(self.connection, self.telemetry, message_receiver=self.message_receiver)
            self.failsafe = Failsafe(self.connection, self.control, self.telemetry, self.message_receiver)
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            if self.message_receiver:
                ...
                logger.info("Central message receiver stopped")
    ...
    def status(self) -> Dict[str, Any]:
        ...
        if self._initialized and self.connection.is_connected():
            ...
            status["subsystems"] = {
                "message_receiver": self.message_receiver is not None,
            ...
            # Add message receiver statistics if available
            if self.message_receiver:
                try:
                    status["message_receiver"] = self.message_receiver.get_statistics()
                except Exception as e:
                    status["message_receiver"] = f"Error: {e}"
        return status
